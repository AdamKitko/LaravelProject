...
<main class="flex-grow pl-10 pr-10 pt-2 font-bold">
    ...
    <div>
        ...
        @foreach($services as $service)
            <div class="inline-block m-5 border pl-5 pr-5 pt-2 pb-2 service-div cursor-pointer" data-modal-id="serviceModal_{{ $loop->index }}">
                {{ $service->name }}
            </div>
            <div id="serviceModal_{{ $loop->index }}" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center service-modal">
                <div class="bg-white rounded-lg overflow-hidden shadow-xl max-w-md w-full">
                    ...
                    <div class="p-4 bg-gray-100 flex justify-end">
                        ...
                        @if($loggedInUserId === $company->user_id)
                            <button class="bg-yellow-500 text-white px-4 py-2 rounded mr-4 edit-service" data-modal-id="editModal_{{ $loop->index }}">Upraviť službu</button>
                        @endif
            ...
            <div id="editModal_{{ $loop->index }}" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center service-modal">
                <div class="bg-white rounded-lg overflow-hidden shadow-xl max-w-md w-full">
                    <div class="p-4 text-center">
                        <h2 class="text-xl font-bold mb-4">Edit Service</h2>
                        <form action="{{ route('services.update', ['id' => $service->id]) }}" method="POST" class="edit-service-form">
                            @csrf
                            @method('PUT')
                            <input type="hidden" name="company_id" value="{{ $company->id }}">
                            <div class="mb-4">
                                <input type="text" name="name" placeholder="Service Name" value="{{ $service->name }}" class="w-full px-4 py-2 border rounded">
                            </div>
                            <div class="mb-4">
                                <input type="text" name="price" placeholder="Service Price" value="{{ $service->price }}" class="w-full px-4 py-2 border rounded">
                            </div>
                            <div class="mb-4">
                                <input type="text" name="length" placeholder="Service Length" value="{{ $service->length }}" class="w-full px-4 py-2 border rounded">
                            </div>
                            <div class="flex justify-end">
                                <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded mr-4">Save Changes</button>
    ...
    @if($loggedInUserId === $company->user_id)
        ...
        <div id="addServiceModal" class="hidden fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center">
            <div class="bg-white rounded-lg overflow-hidden shadow-xl max-w-md w-full">
                <div class="p-4 text-center">
                    <h2 class="text-xl font-bold mb-4">Add New Service</h2>
                    <form action="{{ route('services.store', ['city' => $company->city, 'name' => $company->name, 'id' => $company->id]) }}" method="POST">
                        @csrf
                        <input type="hidden" name="company_id" value="{{ $company->id }}">
                        <div class="mb-4">
                            <input type="text" name="name" placeholder="Service Name" class="w-full px-4 py-2 border rounded">
                        </div>
                        <div class="mb-4">
                            <input type="text" name="price" placeholder="Service Price" class="w-full px-4 py-2 border rounded">
                        </div>
                        <div class="mb-4">
                            <input type="text" name="length" placeholder="Service Length (minutes)" class="w-full px-4 py-2 border rounded">
                        </div>
                        <div class="flex justify-end">
                            <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded mr-4">Add Service</button>
                            <button type="button" class="bg-red-500 text-white px-4 py-2 rounded close-modal">Cancel</button>
                        </div>
                    </form>
                </div>
